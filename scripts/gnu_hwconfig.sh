...
hwcfg=$(dd if="${FILE}" bs=1 skip=$((0x80000)) count=128 2>/dev/null|hexdump -v -e '1/1 "%02x\n"')
hwcfg_data=${hwcfg#*00?}
    ...
    case $line in
      *=[*)
        ...
        c=0;for i in $line;do
          [ $c = $((0x$value)) ] && {
            # FILE=/home/build/inkbox/glohd-n437-bootmmc.bin ./gnu_hwconfig.sh all | sed 's/\[.*\] //g' | sed 's/^/gptNtxHwCfg->m_val.b/' | sed 's/$/;/g'
            # echo "$c"; break
            echo "$i"; break
          }
          c=$((c+1))
