...
int fit_conf_get_node(const void *fit, const char *conf_uname)
{
	...
	if (conf_uname != NULL){
		printf("[NTX] Try to find [%s] \n", conf_uname);
		conf_uname = (char *)fdt_getprop(fit, confs_noffset,conf_uname, &len);
		if (conf_uname == NULL) {
		   ...
		   enum fit_load_op load_op, ulong *datap, ulong *lenp)
{
	...
	if (fit_uname) {
		...
		if (IMAGE_ENABLE_BEST_MATCH && !fit_uname_config) {
			...
			printf("[%s_%d] fit_uname_config:%s\n",__func__,__LINE__,fit_uname_config);
		}
		if (cfg_noffset < 0) {
		...
		printf("[NTX] Using '%s' configuration\n", fit_base_uname_config);
