...
{ 
	...
	//Yian, handle recovery mode detect
	//extern int ntxup_wait_key_esdupg(void);
	//extern int _power_key_status (void);
	printf("Detecting keys for fastboot ...%d %d\n", ntxup_wait_key_esdupg(), _power_key_status());
	if((ntxup_wait_key_esdupg()==1) &&
	   (_power_key_status()!=1))   //disallow power key
	{
		return 1;
	}else{
		return 0;
...
{
	return ntx_check_recovery_cmd_file();
	//return check_and_clean_recovery_flag(); //yian, this is not available on NTX board, MCU always cut power after reboot
}
#endif
