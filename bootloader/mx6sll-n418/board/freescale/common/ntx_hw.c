...
#if 1 //[
	printf("Particular charger detecting .");
	iTryCnt = 0;
	bVal = 0;
	do {
		bRegAddr = 0xDB;
...
void RC5T619_regulator_init(void)
{
	...
	if ((74==gptNtxHwCfg->m_val.bPCB && 1==gptNtxHwCfg->m_val.bPCB_LVL) ||
	    (101==gptNtxHwCfg->m_val.bPCB && 1==gptNtxHwCfg->m_val.bPCB_LVL))
	{
		// PCBA desinged for Ricoh1145 .
		// E60K02BX ,E70K14BX.
	}
	else {
	...
	if(1==gptNtxHwCfg->m_val.bPMIC) {
		...
		if(iChk>=0) {
			if(bReg&0xc0) 
			{
				...
				{
					unsigned char bRegChgDet,bGC_DET,bDCD_timeout,bVBUS_TYPE;
					...
					do {
						...
						iChk = RC5T619_read_reg(0xDA,&bRegChgDet);	// GCHGDET_REG
						if(iChk<0) {
							printf("reading GCHG_DET_REG error !!\n");
							continue;
						}
						bGC_DET = (unsigned char)((bRegChgDet&0xc)>>2);
						bVBUS_TYPE = (unsigned char)((bRegChgDet&0x30)>>4);
						bDCD_timeout = (bRegChgDet&0x80) ? 1:0;
						printf("[%02x],bGC_DET=%x,DCD_TOUT=%x,VBUS_TYPE=%x\n",
								bRegChgDet,bGC_DET,bDCD_timeout,bVBUS_TYPE); 

						if( 0x01==bGC_DET ) // GC_DET . 
						{
							// charger is detecting ...
							udelay(10*1000);
							printf(".");
							continue;
						}
						else if( 0x02==bGC_DET ) // GC_DET . 
						{
							printf("done\n");
						}
#if 1
						else if(bDCD_timeout) { // DCD timeout .
							printf("DCD timeout! \n");
							RC5T619_read_reg(0xDB,&bReg);	// PCHGDET_REG
							printf("PCHGDET_REG=0x%02x\n",bReg);
						}
#endif
						else {
							if(bRegChgDet&0x40) { // IRP
								printf("Detected the irregular port \n");
							}
							else {
								//printf("Detecting the port or detected standard USB battery charging port \n");
							}
							udelay(10*1000);
							RC5T619_charger_redetect();
							printf("%s():Charge detecting ...\n",__FUNCTION__);
							continue ;
						}

						if(0x01==bVBUS_TYPE) {
							//printf ("set charger params for CDP (%d)\n",iTryCnt);
							RC5T619_set_charger_params(USB_CHARGER_CDP);
							iRet = USB_CHARGER_CDP;
						}
						else if (0x02==bVBUS_TYPE) {
							//printf ("set charger params for DCP (%d)\n",iTryCnt);
							RC5T619_set_charger_params(USB_CHARGER_DCP);
							iRet = USB_CHARGER_DCP;
						}
						else {
							//printf ("set charger params for SDP (0x%02x)(%d)\n",bVBUS_TYPE,iTryCnt);
