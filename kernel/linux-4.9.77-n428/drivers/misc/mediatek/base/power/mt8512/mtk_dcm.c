...
#define SPM_NODE		"mediatek,mt8512-consys"
...
#define SPM_BASE		(spm_base)/*0x10006000*/

/* SPM */
#define SPM_SRAM_POOL			(SPM_BASE + 0x0368)
...
void dcm_set_default(unsigned int type)
{
	...
	reg_write(SPM_SRAM_POOL, reg_read(SPM_SRAM_POOL) | 0xf000);
...
static int mt_dcm_dts_map(void)
{
	...
	/* spm */
	node = of_find_compatible_node(NULL, NULL, SPM_NODE);
	if (!node) {
		dcm_info("info:cannot find spm node " SPM_NODE);
		WARN_ON(1);
		goto fail;
	}
	spm_base  = of_iomap(node, 3);
	if (!spm_base) {
		dcm_info("info:cannot iomap spm " SPM_NODE);
		WARN_ON(1);
		goto fail;
	}
