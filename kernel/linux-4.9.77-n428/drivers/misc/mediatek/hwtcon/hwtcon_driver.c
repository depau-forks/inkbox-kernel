...
static int hwtcon_driver_init_device_info(struct platform_device *pdev)
{
	...
	if (hw_tcon_get_epd_type())
		g_hwtcon_device_info.reserved_buf_ready = false;/* cfa panel use new buffer*/
	else {
	...
	void *dev)
{
	 ...
	 * ...
	 */
	TCON_LVL_LOG(TCON_LVL_PIPELINE,"DPI 0x%08x 0x%08x wb input: 0x%08x wf_lut output: 0x%08x",
	...
	void *dev)
{
	...
	TCON_LVL_LOG(TCON_LVL_PIPELINE,"release wf_lut 0x%016llx", released_lut);
...
void hwtcon_driver_cmdq_lut_release_start_loop(void)
{
	...
	TCON_LVL_LOG(TCON_LVL_PIPELINE,"start trigger loop");
...
void hwtcon_driver_cmdq_lut_release_stop_loop(void)
{
	...
	TCON_LVL_LOG(TCON_LVL_PIPELINE,"stop trigger loop");
...
static int regal_major;
static int regal_minor;
struct device *regal_cinfo_dev;
struct device *regal_image_dev;
struct device *regal_tmp_dev;
struct device *regal_wb_dev;
struct device *regal_wf_dev;

static struct class  *regal_class;

static struct cdev  *cdev_cinfo;
static struct cdev  *cdev_image;
static struct cdev  *cdev_tmp;
static struct cdev  *cdev_wb;
static struct cdev  *cdev_wf;

static const struct file_operations  cinfo_ops = {
	.owner = THIS_MODULE,
	.mmap = hwtcon_cinfo_mmap,
};

static const struct file_operations  image_ops = {
	.owner = THIS_MODULE,
	.mmap = hwtcon_image_mmap,
};

static const struct file_operations  tmp_ops = {
	.owner = THIS_MODULE,
	.mmap = hwtcon_tmpbuf_mmap,
};

static const struct file_operations  wb_ops = {
	.owner = THIS_MODULE,
	.mmap = hwtcon_working_buffer_mmap,
};

static const struct file_operations  wf_ops = {
	.owner = THIS_MODULE,
	.mmap = hwtcon_waveform_mmap,
};

static int hwtcon_mapping_resource_for_regal(void)
{
	int err;
	dev_t dev_cinfo;
	dev_t dev_image;
	...
	dev_cinfo = MKDEV(regal_major, regal_minor);
	err = alloc_chrdev_region(&dev_cinfo, 0, 5, "regal");
	if (err) {
		TCON_ERR("alloc regal chrdev region fail");
		return -1;
	}

	regal_major = MAJOR(dev_cinfo);
	regal_minor = MINOR(dev_cinfo);

	dev_image = MKDEV(regal_major, regal_minor + 1);
	dev_tmp = MKDEV(regal_major, regal_minor + 2);
	dev_wb = MKDEV(regal_major, regal_minor + 3);
	dev_wf = MKDEV(regal_major, regal_minor + 4);

	cdev_cinfo = cdev_alloc();
	if (cdev_cinfo == NULL) {
		TCON_ERR("cdev_info alloc fail");
		return -1;
	}

	cdev_init(cdev_cinfo, &cinfo_ops);
	cdev_cinfo->owner = THIS_MODULE;
	err = cdev_add(cdev_cinfo, dev_cinfo, 1);
	if (err) {
		cdev_cinfo = NULL;
		TCON_ERR("cdev_info add fail");
		return -1;
	}

	cdev_image = cdev_alloc();
	if (cdev_image == NULL) {
		TCON_ERR("cdev_image alloc fail");
		return -1;
	}

	cdev_init(cdev_image, &image_ops);
	cdev_image->owner = THIS_MODULE;
	err = cdev_add(cdev_image, dev_image, 1);
	if (err) {
		cdev_image = NULL;
		TCON_ERR("cdev_image add fail");
		return -1;
	}

	cdev_tmp = cdev_alloc();
	if (cdev_tmp == NULL) {
		TCON_ERR("cdev_tmp alloc fail");
		return -1;
	}

	cdev_init(cdev_tmp, &tmp_ops);
	cdev_tmp->owner = THIS_MODULE;
	err = cdev_add(cdev_tmp, dev_tmp, 1);
	if (err) {
		cdev_tmp = NULL;
		TCON_ERR("cdev_tmp add fail");
		return -1;
	}

	cdev_wb = cdev_alloc();
	if (cdev_wb == NULL) {
		TCON_ERR("cdev_wb alloc fail");
		return -1;
	}

	cdev_init(cdev_wb, &wb_ops);
	cdev_wb->owner = THIS_MODULE;
	err = cdev_add(cdev_wb, dev_wb, 1);
	if (err) {
		cdev_wb = NULL;
		TCON_ERR("cdev_wb add fail");
		return -1;
	}

	cdev_wf = cdev_alloc();
	if (cdev_wf == NULL) {
		TCON_ERR("cdev_wf alloc fail");
		return -1;
	}
	cdev_init(cdev_wf, &wf_ops);
	cdev_wf->owner = THIS_MODULE;
	err = cdev_add(cdev_wf, dev_wf, 1);
	if (err) {
		cdev_wf = NULL;
		TCON_ERR("cdev_wf add fail");
		return -1;
	}

	regal_class = class_create(THIS_MODULE, "regal_class");
	if (IS_ERR(regal_class)) {
		cdev_del(cdev_cinfo);
		cdev_del(cdev_image);
		cdev_del(cdev_tmp);
		cdev_del(cdev_wb);
		cdev_del(cdev_wf);
		TCON_ERR("creat regal_class fail");
		return -1;
	}

	regal_cinfo_dev = device_create(
		regal_class,
		NULL,
		dev_cinfo,
		NULL,
		"regal_cinfo"); /* /dev/regal_info */
	if (IS_ERR(regal_cinfo_dev)) {
		class_destroy(regal_class);
		cdev_del(cdev_cinfo);
		cdev_del(cdev_image);
		cdev_del(cdev_tmp);
		cdev_del(cdev_wb);
		cdev_del(cdev_wf);		
		TCON_ERR("cfa creat /dev/regal_info fail");
		return -1;
	}

	regal_image_dev = device_create(
		regal_class,
		NULL,
		dev_image,
		NULL,
		"regal_img"); /* /dev/regal_img */
	if (IS_ERR(regal_image_dev)) {
		class_destroy(regal_class);
		class_destroy(regal_class);
		cdev_del(cdev_cinfo);
		cdev_del(cdev_image);
		...
		cdev_del(cdev_wb);
		cdev_del(cdev_wf);		
		TCON_ERR("cfa creat /dev/regal_img fail");
		return -1;
	}

	regal_tmp_dev = device_create(
		regal_class,
		NULL,
		dev_tmp,
		NULL,
		"regal_tmp"); /* /dev/regal_tmp */
	if (IS_ERR(regal_tmp_dev)) {
		class_destroy(regal_class);
		class_destroy(regal_class);
		cdev_del(cdev_cinfo);
		cdev_del(cdev_image);
		...
		cdev_del(cdev_wb);
		cdev_del(cdev_wf);		
		TCON_ERR("cfa creat /dev/regal_tmp fail");
		return -1;
	}

	regal_wb_dev = device_create(
		regal_class,
		NULL,
		dev_wb,
		NULL,
		"regal_wb"); /* /dev/regal_wb */
	if (IS_ERR(regal_wb_dev)) {
		class_destroy(regal_class);
		class_destroy(regal_class);
		cdev_del(cdev_cinfo);
		cdev_del(cdev_image);
		...
		cdev_del(cdev_wb);
		cdev_del(cdev_wf);		
		TCON_ERR("cfa creat /dev/regal_wb fail");
		return -1;
	}

	regal_wf_dev = device_create(
		regal_class,
		NULL,
		dev_wf,
		NULL,
		"regal_waveform"); /* /dev/regal_waveform */
	if (IS_ERR(regal_wb_dev)) {
		class_destroy(regal_class);
		class_destroy(regal_class);
		cdev_del(cdev_cinfo);
		...
		cdev_del(cdev_wb);
		cdev_del(cdev_wf);		
		TCON_ERR("cfa creat /dev/regal_waveform fail");
		return -1;
	}	
	return 0;
}

void hwtcon_driver_init_working_buffer(void)
{
	memset(hwtcon_fb_info()->wb_buffer_va, 0xff, hwtcon_fb_info()->wb_buffer_size);
	if (hw_tcon_get_epd_type())
		hwtcon_fb_clean_cache(hwtcon_fb_info()->wb_cache_buf);
}

#if 0
static int hwtcon_poweroff(struct device *pDevice)
{
	TCON_LOG("enter hwtcon poweroff");
...
static void hwtcon_shutdown(struct platform_device *pdev)
{
	TCON_LOG("enter hwtcon_shutdown");
	hwtcon_fb_poweroff(&pdev->dev);
}
...
static int hwtcon_probe(struct platform_device *pdev)
{
	...
	hwtcon_driver_init_working_buffer();
	if (hw_tcon_get_epd_type())/*if it is color panel, need mmap for regal*/
		hwtcon_mapping_resource_for_regal();
