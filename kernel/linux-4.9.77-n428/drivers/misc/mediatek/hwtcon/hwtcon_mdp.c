...
int hwtcon_mdp_convert(struct hwtcon_task *task)
{
	...
	*/
	...
	TCON_LVL_LOG(TCON_LVL_MDP,"cfa_mode:%d",cfa_mode);
	...
	if (hw_tcon_get_epd_type()) {
		if ( (HWTCON_FLAG_CFA_SKIP&task->update_data.flags) || (0==cfa_mode) ) 
		{
			...
			TCON_LVL_LOG(TCON_LVL_MDP,"mdp handle normal");
			//dst_buffer_pa += 2;
		}
	} else
		src_format = hwtcon_fb_get_cur_color_format();
	...
	if (hwtcon_core_invert_fb()) 
       ...
       TCON_LVL_LOG(TCON_LVL_MDP,"[MARKER]:%d MDP convert, dither:%d, invert:%d",
		task->update_data.update_marker,
               dither_enable, invert_enable);
	TCON_LVL_LOG(TCON_LVL_MDP,"rotate:%d degree gamma:0x%08x",
		 rotate, gamma_flag);
	//if (rotate) 
	{
		TCON_LVL_LOG(TCON_LVL_MDP,"[MARKER]:%d src width:%d height:%d",
			task->update_data.update_marker,
			src_buffer_width, src_buffer_height);
		TCON_LVL_LOG(TCON_LVL_MDP,"region:[%d %d %d %d]",
			src_region.x,
			src_region.y,
			src_region.width,
			src_region.height);
		TCON_LVL_LOG(TCON_LVL_MDP,"MARKER:%d dst width:%d height:%d",
			task->update_data.update_marker,
			dst_buffer_pitch, dst_buffer_height);
		TCON_LVL_LOG(TCON_LVL_MDP,"region:[%d %d %d %d]",
			dst_region.x,
	...
	if (hwtcon_core_use_cfa_color_and_regal(task)) {
		dither_enable = 1;
		dither_mode = MDP_DITHER_ALGO_Y8_Y4_S;
		gamma_flag = 0;
	...
	easy_mtk_mdp_func_v2(src_buffer_width, src_buffer_height,
				...
				invert_enable,
				rotate, gamma_flag, hw_tcon_get_edp_fliph(), 0, 0);
...
int hwtcon_mdp_memcpy(dma_addr_t dst_buffer, dma_addr_t src_buffer)
{
	...
	return easy_mtk_mdp_func_v2(buffer_width,
				...
				0, 0, 0, hw_tcon_get_edp_fliph(), 0, 0);
}
...
int hwtcon_mdp_regal_handle(struct hwtcon_task *task)
{
	u32 src_buffer_pa = 0;
	u32 dst_buffer_pa = 0;
	u32 src_buffer_width = 0;
	...
	struct rect dst_region = {0};
	u32 src_format = V4L2_PIX_FMT_Y8;
	u32 dst_format = V4L2_PIX_FMT_Y8;
	u8 dither_enable = 0;
	u8 invert_enable = 0;
	...
	//src_region.x = 0;
	//src_region.x = 0;
	//src_region.x = hwtcon_fb_get_width();
	//src_region.x = hwtcon_fb_get_height();
	...
	hwtcon_core_get_task_buffer_info(task, &dst_buffer_pa,
			&dst_buffer_pitch, &dst_buffer_height);
	TCON_LOG("Regal MARKER:%d dst width:%d height:%d region:[%d %d %d %d]",
		task->update_data.update_marker,
		dst_buffer_pitch, dst_buffer_height,
		dst_region.x,
		dst_region.y,
		dst_region.width,
		dst_region.height);
	ret = easy_mtk_mdp_func_v2(src_buffer_width, src_buffer_height,
		dst_buffer_pitch, dst_buffer_height,
		src_format, dst_format,
		...
		invert_enable,
		rotate, gamma_flag, hw_tcon_get_edp_fliph(), 0, 1);
	return ret;
}
