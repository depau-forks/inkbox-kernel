...
u32 hwtcon_fb_get_cur_color_format(void)
{
	...
	case 24:
		...
		break;
	case 32:
		if(V4L2_PIX_FMT_ARGB32==hwtcon_fb_info()->mdp_src_format) {
		...
		struct fb_info *info)
{
	...
	switch (hwtcon_device_info()->color_format) {
		...
		case EPD_COLOR_FORMAT_ARGB32:
			...
			if( (V4L2_PIX_FMT_ARGB32!=((struct fb_private_info *)(info->par))->mdp_src_format) &&
			  (V4L2_PIX_FMT_ABGR32!=((struct fb_private_info *)(info->par))->mdp_src_format) )
			{
...
static int hwtcon_fb_ioctl_send_update(void *arg)
{
	int status = 0;
	struct hwtcon_update_data update_data;
	enum WAVEFORM_MODE_ENUM wfm_user;
	unsigned long dwJiffiesNow = jiffies;
	static unsigned long giLastSendUpdJiffies ;
	...
	wfm_user = update_data.waveform_mode;
	update_data.waveform_mode = wf_lut_wfmode_user2driver(wfm_user);

	if(-1!=hwtcon_fb_info()->force_update_mode) {
		update_data.update_mode = hwtcon_fb_info()->force_update_mode;
	}

	//TCON_LVL_LOG(TCON_LVL_USER,"user update %u [%d %d %d %d] wfm=%d(%s)->%d(%s),%s %s,flags=0x%x,dither=0x%x",
	TCON_LVL_LOG(TCON_LVL_USER0,"user update %u [%d %d %d %d] wfm=%d(%s)->%d(%s),%s %s,flags=0x%x,dither=0x%x,upd interval=%dms",
			update_data.update_marker,
			update_data.update_region.top,update_data.update_region.left,
			update_data.update_region.width,update_data.update_region.height,
			wfm_user,hwtcon_core_get_wf_mode_name(wfm_user),
			update_data.waveform_mode,hwtcon_core_get_wf_mode_name(update_data.waveform_mode),
			(-1!=hwtcon_fb_info()->force_update_mode)?"force":"",
			update_data.update_mode?"full":"partial",
			update_data.flags,update_data.dither_mode,
			jiffies_to_msecs(dwJiffiesNow-giLastSendUpdJiffies));

	status = hwtcon_core_submit_task(&update_data);
	giLastSendUpdJiffies = dwJiffiesNow;
	return status;
}
...
{
	int iRet ;
	char *pszCFA_mode_name;


	if(0==hw_tcon_get_epd_type()) {
		TCON_ERR("cannot set cfa mode on non CFA panel .");
		return -2;
	}
	
	pszCFA_mode_name = cfa_get_mode_name_by_id(cfa_mode);
	if(pszCFA_mode_name) {
...
static int hwtcon_fb_ioctl_set_cfa_mode(void *arg)
{
	...
	if (copy_from_user(&cfa_mode, (void *)arg,sizeof(cfa_mode)) != 0) {
		iChk = hwtcon_fb_set_cfa_mode(cfa_mode);
		if(iChk<0) {
			return -1;
		}
	}
...
int hwtcon_cinfo_mmap(struct file *flip, struct vm_area_struct *vma)
{
	int status = 0;
	...
	if (hw_tcon_get_epd_type()) {
		...
		table = hwtcon_fb_info()->cinfo_cache_buf->sgt;
		addr = vma->vm_start;
		offset = vma->vm_pgoff * PAGE_SIZE;
		for_each_sg(table->sgl, sg, table->nents, i) {
			struct page *page = sg_page(sg);
...
int hwtcon_image_mmap(struct file *flip, struct vm_area_struct *vma)
{
	int status = 0;
	...
	if (hw_tcon_get_epd_type()) {
		table = hwtcon_fb_info()->img_cache_buf->sgt;
		addr = vma->vm_start;
		offset = vma->vm_pgoff * PAGE_SIZE;
...
int hwtcon_tmpbuf_mmap(struct file *flip, struct vm_area_struct *vma)
{
	int status = 0;
	...
	if (hw_tcon_get_epd_type()) {
		table = hwtcon_fb_info()->temp_img_cache_buf->sgt;
		addr = vma->vm_start;
		offset = vma->vm_pgoff * PAGE_SIZE;
...
int hwtcon_working_buffer_mmap(struct file *flip, struct vm_area_struct *vma)
{
	int status = 0;
	...
	if (hw_tcon_get_epd_type()) {
		table = hwtcon_fb_info()->wb_cache_buf->sgt;
		addr = vma->vm_start;
		offset = vma->vm_pgoff * PAGE_SIZE;
...
int hwtcon_waveform_mmap(struct file *flip, struct vm_area_struct *vma)
{
	int status = 0;

	vma->vm_page_prot = pgprot_noncached(vma->vm_page_prot);
	/* vma->vm_pgoff << PAGE_SHIFT */
	status = dma_mmap_attrs(hwtcon_fb_info()->dev,
		vma,
	...
	struct fb_private_info *private_info, struct platform_device *pdev)
{
	...
	private_info->mdp_src_format = V4L2_PIX_FMT_ABGR32;// RGBA8888
	private_info->force_update_mode = -1;
	...
	if (hwtcon_device_info()->reserved_buf_ready) {
		...
		if (hw_tcon_get_epd_type()) {
			private_info->wb_cache_buf = hwtcon_fb_alloc_cached_mva_buf(&pdev->dev,
				private_info->wb_buffer_size,
				GFP_KERNEL);
			if (private_info->wb_cache_buf == NULL) {
			...
			private_info->wb_buffer_va = private_info->wb_cache_buf->alloc_va;
			private_info->wb_buffer_pa = private_info->wb_cache_buf->dma_handle;
		} else {
...
static ssize_t force_update_mode_read(struct device *dev, struct device_attribute *attr,char *buf);
static ssize_t force_update_mode_write(struct device *dev, struct device_attribute *attr,const char *buf, size_t count);
static DEVICE_ATTR(force_update_mode, S_IWUSR | S_IRUSR|S_IROTH|S_IRGRP,
	   	force_update_mode_read, force_update_mode_write);

static ssize_t force_update_mode_read(struct device *dev, struct device_attribute *attr,char *buf)
{
	sprintf (buf,"%d\n",hwtcon_fb_info()->force_update_mode);
	//printk("%s():%s=%d\n",__FUNCTION__,buf,(int)(hwtcon_fb_info()->force_update_mode));
	return strlen(buf);
}

static ssize_t force_update_mode_write(struct device *dev, struct device_attribute *attr,
		       const char *buf, size_t count)
{
	sscanf(buf,"%d",&hwtcon_fb_info()->force_update_mode);
...
static ssize_t waveform_modes_read(struct device *dev, struct device_attribute *attr,char *buf);
static ssize_t waveform_modes_write(struct device *dev, struct device_attribute *attr,const char *buf, size_t count);
static DEVICE_ATTR(waveform_modes, S_IWUSR | S_IRUSR|S_IROTH|S_IRGRP,
	   	waveform_modes_read, waveform_modes_write);
...
static ssize_t waveform_modes_read(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	len = sprintf (buf_ptr,"%s\n",wf_lut_get_waveform_modes_info_header());	total_len += len;
	buf_ptr += len;

	do {
		info_str = wf_lut_get_waveform_modes_info((0==items)?1:0);
		if(info_str) {
			len = sprintf (buf_ptr,"%s\n",info_str);total_len += len;
			buf_ptr += len;
			items++;
		}
	} while(info_str);
...
static ssize_t waveform_modes_write(struct device *dev, struct device_attribute *attr,
		       const char *buf, size_t count)
{
	char szWFM_name[32];
	int iWFM_user,iWFM_driver,iWFM_eink;

	sscanf(buf,"%s , %d , %d , %d",szWFM_name,&iWFM_user,&iWFM_driver,&iWFM_eink);
	TCON_LVL_LOG(TCON_LVL_USER,"waveform mode \"%s\",user=%d,driver=%d,eink=%d",
			szWFM_name,iWFM_user,iWFM_driver,iWFM_eink);

	wf_lut_wfmode_set_driver_wfm(iWFM_user,iWFM_driver);
...
static u32 _getMDPSrcFmtIdByName(const char *szFmtName)
{
	...
	if (dwRet!=(u32)-1) {
		if(0==strcmp(szFmtName,"RGBA32")) {
			dwRet = V4L2_PIX_FMT_ABGR32;
		}
	}
	return dwRet;
}
		       ...
		       const char *buf, size_t count)
{
	...
	if(dwMDPSrcId!=(u32)-1) {
		...
		case V4L2_PIX_FMT_Y8:
			...
			hwtcon_device_info()->color_format = EPD_COLOR_FORMAT_Y8;
			break;
		case V4L2_PIX_FMT_RGB565:
			...
			hwtcon_device_info()->color_format = EPD_COLOR_FORMAT_RGB565;
			break;
		case V4L2_PIX_FMT_ARGB32:
			...
			g_framebuffer_info->var.grayscale = 0;
			g_framebuffer_info->var.nonstd = 0;
			hwtcon_device_info()->color_format = EPD_COLOR_FORMAT_ARGB32;
			break;
		case V4L2_PIX_FMT_ABGR32:
			...
			g_framebuffer_info->var.grayscale = 0;
			g_framebuffer_info->var.nonstd = 0;
			hwtcon_device_info()->color_format = EPD_COLOR_FORMAT_RGBA8888;
			break;
		...
		if(hwtcon_fb_check_var(&g_framebuffer_info->var,g_framebuffer_info)<0) {
			TCON_ERR("\"%s\" format check error !\n",cMDPSrcFmtA);
		}
	}
	else {
		TCON_ERR("\"%s\" not supported !\n",cMDPSrcFmtA);
		_dumpMDPSrcFmt();
	}
...
static void hwtcon_fb_create_sys_attr(struct device * fb_dev)
{
	...
	if( hw_tcon_get_epd_type() ) {
		err = sysfs_create_file(&fb_dev->kobj, &dev_attr_cfa_mode.attr);
		if (err) {
			TCON_ERR("Can't create cfa_mode attr sysfs !\n");
		}
	}

	err = sysfs_create_file(&fb_dev->kobj, &dev_attr_waveform_modes.attr);
	if (err) {
		TCON_ERR("Can't create waveform_modes attr sysfs !\n");
	}

	err = sysfs_create_file(&fb_dev->kobj, &dev_attr_force_update_mode.attr);
	if (err) {
		TCON_ERR("Can't create force_update_mode attr sysfs !\n");
	}
}
...
int hwtcon_fb_poweroff(struct device *dev)
{
	...
	do {
		...
		if( time_is_after_jiffies(hwtcon_fb_info()->jiffies_xon_onoff+msecs_to_jiffies(60)) ) {
			TCON_ERR("%s waiting for vee stable !",__func__);
			dwChkErrors |= 0x10;
		}
