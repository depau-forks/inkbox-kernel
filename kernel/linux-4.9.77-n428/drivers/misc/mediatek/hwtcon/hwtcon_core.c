...
void hwtcon_core_put_task(struct hwtcon_task *task)
{
	...
	hwtcon_debug_record_printf(
		" regal:%s wfm:%02d->%s(%d) wf_cnt:%d flags=0x%x ",
		(task->regal_status ==
	...
	hwtcon_debug_record_printf(
		"trigger_wf_lut %03d wf_lut_done total:%03d marker:%u",
	...
	list_for_each_entry_safe(update_marker, tmp,
		...
		TCON_LOG("[MARKER] update_completion: %u",
	...
	list_for_each_entry_safe(update_marker, tmp,
		...
		if (signal_marker) {
			...
			TCON_LOG("[MARKER] update_completion: %u",
				update_marker->update_marker);
			TCON_LOG("[%u] update end marker=%u,",
				update_marker->update_marker,
				update_marker->update_marker);
...
void hwtcon_core_handle_lut_release_timeout_cb(unsigned long param)
{
	...
	if (!release_task_found)
		...
		TCON_ERR("LUT[%d] timeout on task marker[%u] region[%d %d %d %d]",
			task->assign_lut,
			task->update_data.update_marker,
			hwtcon_core_get_task_region(task).x,
			hwtcon_core_get_task_region(task).y,
...
int hwtcon_core_wait_for_task_triggered(u32 update_marker)
{
	...
	TCON_LOG("wait marker [%u] submit",update_marker);
	...
	if (found) {
		...
		TCON_LOG("waiting for marker [%u] submit",update_marker);
		if (wait_for_completion_timeout(
				&marker->submit_completion,
				msecs_to_jiffies(
				HWTCON_TASK_WAIT_MARKER_TIMEOUT_MS)) == 0) {
			TCON_ERR("wait marker[%u] submit timeout:",
			...
			TCON_LOG("waiting for marker [%u] submit done",update_marker);
		}
...
int hwtcon_core_wait_for_task_displayed(u32 update_marker)
{
	...
	TCON_LOG("wait marker[%u]",update_marker);
	/* search global marker list */
	spin_lock_irqsave(&hwtcon_fb_info()->fb_global_marker_list.lock, flags);
	list_for_each_entry_safe(marker, tmp,
		&hwtcon_fb_info()->fb_global_marker_list.list, global_list) {
	...
	if (found) {
		...
		TCON_LOG("waiting for marker[%u]",marker->update_marker);
		if (wait_for_completion_timeout(
				&marker->update_completion,
				msecs_to_jiffies(
				HWTCON_TASK_WAIT_MARKER_TIMEOUT_MS)) == 0) {
			TCON_ERR("wait marker[%u] update timeout:",
				marker->update_marker);
			TCON_ERR("mask[0x%016llx] state[%d]",
				marker->lut_mask,
				marker->marker_state);
			...
			TCON_LOG("waiting for marker[%u] done",marker->update_marker);
		}
...
int hwtcon_core_submit_task(struct hwtcon_update_data *update_data)
{
	...
	TCON_LOG("SUBMIT:task:0x%llx marker:%u wf_mode:%d(%s)",
		task->unique_id,
		task->update_data.update_marker,
		task->update_data.waveform_mode,
		hwtcon_core_get_wf_mode_name(update_data->waveform_mode));
	TCON_LOG("[%u] update start marker=%u, start time=%lld",
				task->update_data.update_marker,
				task->update_data.update_marker,
				task->time_submit);
	...
	struct work_struct *work_item)
{
	...
	if (hwtcon_debug_get_info()->enable_dump_next_buffer>0) {
		if(hwtcon_fb_info()->debug_img_buffer_counter>0) {
			snprintf(hwtcon_fb_info()->debug_img_buffer_name[0],
				MAX_FILE_NAME_LEN,
			"%s/next_wb_%d_m%04u_%dx%dy%dw%dh.bin",
	...
	if (hwtcon_debug_get_info()->collision_debug) {
		...
		enum WF_SLOT_ENUM slot = wf_lut_get_waveform_mode_slot(
			task->update_data.waveform_mode,
			task->night_mode,&eink_wfm);

		TCON_LOG("eink wf slot=%d,mode=%d",slot,eink_wfm);
	...
	u32 *buffer_pa, u32 *buffer_width, u32 *buffer_height)
{
	...
	if (hwtcon_mdp_need_cfa(task)) {
		...
		TCON_LVL_LOG(TCON_LVL_CFA,"MDP INPUT CFA");
	}  else {
		if (buffer_pa)
...
static void hwtcon_core_auto_select_waveform_mode(struct hwtcon_task *task_org,struct hwtcon_task *task_new)
{

	TCON_LOG("org waveform_mode=%d,regal_status=%d,night_mode=%d",
			task_org->update_data.waveform_mode,
			task_org->regal_status,
			task_org->night_mode);

	if (hwtcon_core_use_regal(&task_org->update_data, &task_org->regal_mode))
		task_new->regal_status = REGAL_STATUS_REGAL;
	else
		task_new->regal_status = REGAL_STATUS_NON_REGAL;


	if(-1!=hwtcon_fb_info()->force_update_mode) {
		task_new->update_data.update_mode = hwtcon_fb_info()->force_update_mode;
	}
	
	if (hwtcon_core_use_cfa_color_mapping(task_org) && (task_org->regal_status == REGAL_STATUS_REGAL)) {
		if(task_new->night_mode) {
			if(UPDATE_MODE_PARTIAL==task_new->update_data.update_mode) {
				task_new->update_data.waveform_mode = WAVEFORM_MODE_GLKW16;
			}
			else {
				task_new->update_data.waveform_mode = WAVEFORM_MODE_GCK16;
			}
		}
		else 
		{
			if(UPDATE_MODE_PARTIAL==task_new->update_data.update_mode) {
				task_new->update_data.waveform_mode = WAVEFORM_MODE_GLRC16;
			}
			else {
				task_new->update_data.waveform_mode = WAVEFORM_MODE_GCC16;
			}
		}
	} else if (hwtcon_core_use_cfa_color_mapping(task_org) && (task_org->regal_status == REGAL_STATUS_NON_REGAL)) {
		if(task_new->night_mode) {
			if(UPDATE_MODE_PARTIAL==task_new->update_data.update_mode) {
		...
		if(task_new->night_mode) {
			...
			if(UPDATE_MODE_PARTIAL==task_new->update_data.update_mode) {
				task_new->update_data.waveform_mode = WAVEFORM_MODE_GL16;
			}
			else {
				task_new->update_data.waveform_mode = WAVEFORM_MODE_GC16;
			}
		}
	}
	if (hwtcon_core_use_regal(&task_new->update_data, &task_new->regal_mode))
		task_new->regal_status = REGAL_STATUS_REGAL;
	else
		task_new->regal_status = REGAL_STATUS_NON_REGAL;


	TCON_LVL_LOG(TCON_LVL_USER,"driver update %u [%d %d %d %d] wfm=%d(%s)->%d(%s),%s %s,flags=0x%x,dither=0x%x",
			task_new->update_data.update_marker,
			task_new->update_data.update_region.top,task_new->update_data.update_region.left,
			task_new->update_data.update_region.width,task_new->update_data.update_region.height,
			task_org->update_data.waveform_mode,hwtcon_core_get_wf_mode_name(task_org->update_data.waveform_mode),
			task_new->update_data.waveform_mode,hwtcon_core_get_wf_mode_name(task_new->update_data.waveform_mode),
			(-1!=hwtcon_fb_info()->force_update_mode)?"force":"",
			task_new->update_data.update_mode?"full":"partial",
			task_new->update_data.flags,task_new->update_data.dither_mode);
		...
		list_for_each_entry_safe(collision_task, tmp,
			...
			&task_region, &collision_task_region, &merge_region) &&
				...
				collision_task->update_data.waveform_mode) {
					...
					hwtcon_core_auto_select_waveform_mode(collision_task,insert_task);
				}
	...
	list_for_each_entry_safe(task, tmp,
		...
		if (hwtcon_core_can_merge_trigger_task_region(
			...
			if (insert_task->update_data.waveform_mode !=
				...
				hwtcon_core_auto_select_waveform_mode(task,insert_task);
			}
			insert_task->lut_dependency = 0LL;
...
void hwtcon_core_create_collision_task(struct hwtcon_task *task)
{
	...
	hwtcon_core_auto_select_waveform_mode(task , collision_task);
	...
	u32 *regal_mode)
{
	...
	case WAVEFORM_MODE_GLRC16:
		*regal_mode = REGAL_MODE_REGAL_CFA;
		return true;
...
int hwtcon_core_trigger_pipeline(struct hwtcon_task *task)
{
	...
	TCON_LVL_LOG(TCON_LVL_PIPELINE,"[%u] waveform=0x%x (%s) mode=0x%x update region top=%d,",
		task->update_data.update_marker,
		task->update_data.waveform_mode,
		hwtcon_core_get_wf_mode_name(task->update_data.waveform_mode),
		task->update_data.update_mode,
		hwtcon_core_get_task_user_region(task).y);
	TCON_LVL_LOG(TCON_LVL_PIPELINE,"left=%d, width=%d, height=%d flags=0x%x rotation=%d",
			hwtcon_core_get_task_user_region(task).x,
			hwtcon_core_get_task_user_region(task).width,
			hwtcon_core_get_task_user_region(task).height,
			task->update_data.flags, hwtcon_fb_get_rotation());

	TCON_LVL_LOG(TCON_LVL_PIPELINE,"TRIGGER:task:0x%llx marker:%u time:%lld region[%d %d %d %d]",
		task->unique_id,
		task->update_data.update_marker,
	...
	TCON_LVL_LOG(TCON_LVL_PIPELINE,"%s update:%d->%s wf_mode:%d-%s temperature:%d-%d",
	...
	if(!fiti_pmic_judge_power_on()) {
		...
		int iCurrentPolicy = current->policy;
		int iCurrentPriority = task_prio(current);
		
		if( (SCHED_RR==iCurrentPolicy) || (SCHED_FIFO==iCurrentPolicy) ) {
			// force set to normal .
			TCON_LOG("force set policy %d to normal sched policy ...",iCurrentPolicy);
			param.sched_priority = iCurrentPriority ;
		...
		if( (SCHED_RR==iCurrentPolicy) || (SCHED_FIFO==iCurrentPolicy) ) {
			// restore to original .
			TCON_LOG("restore to sched policy %d ",iCurrentPolicy);
			sched_setscheduler(current, iCurrentPolicy, &param);
		}
	}
	...
	if (status != 0) {
		...
		list_for_each_entry_safe(update_marker, tmp,
			&task->marker_info_list.list, task_list) {
			complete(&update_marker->submit_completion);
			TCON_LOG("[MARKER] submit_completion: %u",
				update_marker->update_marker);
			spin_lock_irqsave(&update_marker->marker_lock, flags);
	...
	if (info.update_void) {
		...
		if (info.collision_lut_0 == 0 &&
			...
			list_for_each_entry_safe(update_marker, tmp,
				...
				TCON_LOG("temp index: %d rotation=%d",
					hwtcon_core_read_temp_zone(),
					hwtcon_fb_get_rotation());
				TCON_LOG("task:0x%llx marker:%u update",
					task->unique_id,
					update_marker->update_marker);
			...
			hwtcon_core_auto_select_waveform_mode(task,task);
		...
		list_for_each_entry_safe(update_marker, tmp,
				&task->marker_info_list.list, task_list) {
			TCON_LVL_LOG(TCON_LVL_PIPELINE,"[%u] Sending update. waveform:%d (%s)",
				update_marker->update_marker,
				task->update_data.waveform_mode,
			hwtcon_core_get_wf_mode_name(
					task->update_data.waveform_mode));
			TCON_LVL_LOG(TCON_LVL_PIPELINE,"mode:%d update region top=%d,left=%d,width",
				task->update_data.update_mode,
				hwtcon_core_get_task_user_region(task).y,
				hwtcon_core_get_task_user_region(task).x);
			TCON_LVL_LOG(TCON_LVL_PIPELINE,"=%d, height=%d temp index: %d rotation=%d",
				hwtcon_core_get_task_user_region(task).width,
				hwtcon_core_get_task_user_region(task).height,
				hwtcon_core_read_temp_zone(),
				hwtcon_fb_get_rotation());

			TCON_LVL_LOG(TCON_LVL_PIPELINE,"[%u] Sending update in LUT: %d",
					update_marker->update_marker,
					task->assign_lut);
		}

		TCON_LVL_LOG(TCON_LVL_PIPELINE,"trigger WF_LUT with lut:%d region[%d %d %d %d]",
			task->assign_lut,
			hwtcon_core_get_task_region(task).x,
			hwtcon_core_get_task_region(task).y,
			hwtcon_core_get_task_region(task).width,
			hwtcon_core_get_task_region(task).height);
		TCON_LVL_LOG(TCON_LVL_PIPELINE,"wf_mode:%d lut status[0x%016llx 0x%016llx 0x%016llx]",
			task->update_data.waveform_mode,
			hwtcon_fb_info()->lut_free,
			hwtcon_core_get_released_lut(),
			hwtcon_fb_info()->lut_active);
...
int hwtcon_core_use_cfa_color_and_regal(struct hwtcon_task *task)
{
	if (hw_tcon_get_epd_type() == 0)
		return 0;
	if (hwtcon_fb_get_cur_color_format() == V4L2_PIX_FMT_Y8)
		...
		return 0;
	else if ((0==HWTCON_FLAG_GET_CFA_MODE(task->update_data.flags)) && (hwtcon_fb_info()->cfa_mode<=0))
		return 0;
...
int hwtcon_core_dispatch_mdp(void *ignore)
{
	...
	while (1) {
		...
		TCON_LVL_LOG(TCON_LVL_MDP,"wait for trigger mdp Task");
		...
		TCON_LVL_LOG(TCON_LVL_MDP,"[MARKER]:%u flags:0x%08x to MDP",
			task->update_data.update_marker,
			task->update_data.flags);
		...
		flush_cache_all();
		outer_flush_all();
		//hwtcon_fb_invalid_cache(hwtcon_fb_info()->fb_cache_buf);
		...
		if (hwtcon_debug_get_info()->enable_dump_next_buffer>0) {
			if(hwtcon_fb_info()->debug_img_buffer_counter>0) {
				snprintf(hwtcon_fb_info()->debug_img_buffer_name[0],
					MAX_FILE_NAME_LEN,
				"%s/next_fb_%d_m%04u_%dx%dy%dw%dh.bin",
					hwtcon_fb_info()->debug_img_dump_path,
					hwtcon_fb_info()->debug_img_buffer_counter,
					task->update_data.update_marker,
					hwtcon_core_get_task_region(task).x,
		...
		if (hwtcon_core_use_cfa_color_mapping(task)) {
			...
			if (ret)
				...
				TCON_LVL_LOG(TCON_LVL_CFA,"trigger cfa color rendering begin, cfa_mode=%d, task cfa_mode=%d",
			...
			//hwtcon_fb_clean_cache(hwtcon_fb_info()->img_cache_buf);
		}
		...
		if (hwtcon_debug_get_info()->enable_dump_next_buffer>0) {
			if(hwtcon_fb_info()->debug_img_buffer_counter>0) {
				snprintf(hwtcon_fb_info()->debug_img_buffer_name[0],
					MAX_FILE_NAME_LEN,
				"%s/next_%d_m%04u_%dx%dy%dw%dh.bin",
					hwtcon_fb_info()->debug_img_dump_path,
					hwtcon_fb_info()->debug_img_buffer_counter,
					task->update_data.update_marker,
					hwtcon_core_get_task_region(task).x,
		...
		if (hwtcon_debug_get_info()->enable_dump_image_buffer) {
			...
			snprintf(hwtcon_fb_info()->debug_img_buffer_name[index],
				MAX_FILE_NAME_LEN,
			"/%s/img_%04u_%03d_%03d_%03d_%03d.bin",
				hwtcon_fb_info()->debug_img_dump_path,
				task->update_data.update_marker,
				hwtcon_core_get_task_region(task).x,
				hwtcon_core_get_task_region(task).y,
	...
	while (1) {
		...
		if (task == NULL) {
			spin_unlock_irqrestore(
				...
				flags);
			TCON_LVL_LOG(TCON_LVL_PIPELINE,"wait for trigger pipeline Task");
			wait_woken(&wait,
				TASK_INTERRUPTIBLE,
		...
		TCON_LVL_LOG(TCON_LVL_PIPELINE,"pipeline Task trigged marker %u",
			task->update_data.update_marker);
...
void hwtcon_core_handle_release_lut(int lut_id)
{
	...
	TCON_LOG("DONE:task:0x%llx marker[%u] time:%lld cost:%d ms",
		task->unique_id,
		task->update_data.update_marker,
		task->time_wf_lut_done,
		hwtcon_hal_get_time_in_ms(
...
const char *gszWF_name_init = "init";
const char *gszWF_name_du = "du";
const char *gszWF_name_gc16 = "gc16";
const char *gszWF_name_gl16 = "gl16";
const char *gszWF_name_glr16 = "glr16";
const char *gszWF_name_a2 = "a2";
const char *gszWF_name_gck16 = "gck16";
const char *gszWF_name_glkw16 = "glkw16";
const char *gszWF_name_gcc16 = "gcc16";
const char *gszWF_name_glrc16 = "glrc16";
const char *gszWF_name_auto = "auto";
const char *gszWF_name_unkown = "unkown";
char *hwtcon_core_get_wf_mode_name(enum WAVEFORM_MODE_ENUM mode)
{
	switch (mode) {
	case WAVEFORM_MODE_INIT:
		return (char *)gszWF_name_init;
	case WAVEFORM_MODE_DU:
		return (char *)gszWF_name_du;
	case WAVEFORM_MODE_GC16:
		return (char *)gszWF_name_gc16;
	case WAVEFORM_MODE_GL16:
		return (char *)gszWF_name_gl16;
	case WAVEFORM_MODE_GLR16:
		return (char *)gszWF_name_glr16;
	case WAVEFORM_MODE_A2:
		return (char *)gszWF_name_a2;
	case WAVEFORM_MODE_AUTO:
		return (char *)gszWF_name_auto;
	case WAVEFORM_MODE_GCK16:
		return (char *)gszWF_name_gck16;
	case WAVEFORM_MODE_GLKW16:
		return (char *)gszWF_name_glkw16;
	case WAVEFORM_MODE_GCC16:
		return (char *)gszWF_name_gcc16;
	case WAVEFORM_MODE_GLRC16:
		return (char *)gszWF_name_glrc16;
	default:
		return "unknown_mode";
	}
...
void hwtcon_core_handle_auto_waveform(struct hwtcon_task *task)
{
	...
	if (task->update_data.waveform_mode != WAVEFORM_MODE_AUTO) {
		TCON_LVL_LOG(TCON_LVL_PIPELINE,"[%u]chist=0x%x[%d] nhist=0x%x[%d]",
			task->update_data.update_marker,
	...
	int *acquired_id)
{
	...
	if (free_lut != 0LL) {
		...
		*acquired_id = hwtcon_hal_ffs(free_lut);
		...
		TCON_LVL_LOG(TCON_LVL_PIPELINE,"alloc free lut:%d lut status: 0x%016llx",
			*acquired_id,
			hwtcon_fb_info()->lut_free);
		TCON_LVL_LOG(TCON_LVL_PIPELINE,"0x%016llx 0x%016llx",
			hwtcon_core_get_released_lut(),
			hwtcon_fb_info()->lut_active);
