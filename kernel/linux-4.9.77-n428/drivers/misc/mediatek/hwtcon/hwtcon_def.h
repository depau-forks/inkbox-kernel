...
#define TCON_LVL_USER0		1
#define TCON_LVL_USER		2
#define TCON_LVL_PMIC		3
#define TCON_LVL_CFA		4
#define TCON_LVL_MDP		5
#define TCON_LVL_PIPELINE	6

#define TCON_LVL_LOG(lvl,string,args...) \
do { \
	if (hwtcon_get_log_level()>=lvl) \
		pr_notice("[HWTCON LOG %d]"string"\n",lvl,##args); \
} while (0)

#define TCON_LOG(string, args...) \
	TCON_LVL_LOG(TCON_LVL_USER,string,##args)
