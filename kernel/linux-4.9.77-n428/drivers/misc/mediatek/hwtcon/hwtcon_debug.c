...
void debug_write_log_level(int argc, char *argv[])
{
	...
	g_debug_info.log_level = log_level;
	TCON_ERR("set loglevel to %d", g_debug_info.log_level);
}
...
void debug_dump_power_status(int argc, char *argv[])
{
	...
	list_for_each_entry_safe(update_marker, tmp,
		...
		TCON_ERR("marker[%u] state[%d] lut_mask[0x%016llx]",
...
void debug_dump_next_buffer(int argc, char *argv[])
{
	...
	pr_info("enable dump next buffer en=%d cnt=%d path=%s\n",
...
void debug_dump_image_buffer(int argc, char *argv[])
{
	...
	if(argc>=2) {
		strcpy(hwtcon_fb_info()->debug_img_dump_path,argv[1]);
	}
	else {
		strcpy(hwtcon_fb_info()->debug_img_dump_path,"/tmp");
	}

	TCON_LOG("%s dump image buffer, dump path=\"%s\"",
