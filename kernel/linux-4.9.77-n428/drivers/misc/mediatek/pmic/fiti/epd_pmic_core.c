...
int epd_vcom_enable(int iIsEnable)
{
	int iOldState = -1;
	...
	iOldState = regulator_is_enabled(gptRegulator_vcom);
	if(iIsEnable) {
		if(regulator_enable(gptRegulator_vcom)) {
			FITI_ERR("vcom enable failed !");
		}
		else {
...
void fiti_wait_power_good(void)
{
#ifndef FPGA_EARLY_PORTING
	{
		...
		FITI_LOG("waiting for power good (timeout=%dms)",iWaitPG_TimeoutMS);
