...
//#define CFA_HANDLE_THREAD 2
	...
	#define CFA_LOG(string, args...) \
		TCON_LVL_LOG(TCON_LVL_CFA,"[CFA LOG]"string"\n", ##args); 
#endif
...
static char *cfa_color_info_output2 = NULL;
...
void cfa_color_handle(unsigned char *color_buffer,unsigned char *gray_buffer, 
		...
		unsigned char *c_buffer, unsigned int enhance,
		unsigned int thread_id,	unsigned start_x,unsigned start_y)
{
	int cfa_conv_chk = 0;
	...
	unsigned int color_format = hwtcon_fb_get_cur_color_format();
	int iEinkColorFormat;
	...
	if(V4L2_PIX_FMT_ARGB32==color_format) {
		iEinkColorFormat = ARGB8888;
	}
	else {
		iEinkColorFormat = RGBA8888;
	}
	...
	case HWTCON_CFA_MODE_NTX:
	{
		...
		if(V4L2_PIX_FMT_ARGB32==color_format) {
			dwFlags |= EPDFB_DC_FLAG_32BITS_ARGB;
		}
	...
	case HWTCON_CFA_MODE_EINK_G1:
		...
		cfa_conv_chk = eink_color_mapping_Gain(color_buffer,iEinkColorFormat,gray_buffer,
				iFB_W,iFB_H,c_buffer,1,start_x,start_y,start_x+width-1,start_y+height,iRot);
		break;
	...
	case HWTCON_CFA_MODE_EINK_G2:
		CFA_LOG("width=%d,height=%d,gain=2,left=%d,top=%d,right=%d,bottom=%d,rotate=%d",
			iFB_W,height,start_x,start_y,start_x+width-1,start_y+height,iRot);
		cfa_conv_chk = eink_color_mapping_Gain(color_buffer,iEinkColorFormat,gray_buffer,
				iFB_W,iFB_H,c_buffer,2,start_x,start_y,start_x+width-1,start_y+height,iRot);
		break;
#endif //] defined(HWTCON_CFA_MODE_EINK_NORMAL) || defined(HWTCON_CFA_MODE_EINK_G1)
	...
	case HWTCON_CFA_MODE_EINK_G0:
		CFA_LOG("width=%d,height=%d,gain=0,left=%d,top=%d,right=%d,bottom=%d,rotate=%d",
			iFB_W,height,start_x,start_y,start_x+width-1,start_y+height,iRot);
		cfa_conv_chk = eink_color_mapping_Gain(color_buffer,iEinkColorFormat,gray_buffer,
				iFB_W,iFB_H,c_buffer,0,start_x,start_y,start_x+width-1,start_y+height,iRot);
		break;
#endif //] defined(HWTCON_CFA_MODE_EINK_NORMAL) || defined(HWTCON_CFA_MODE_EINK_G1)
	...
	case HWTCON_CFA_MODE_EINK_AIE_S4:
		CFA_LOG("width=%d,height=%d,left=%d,top=%d,right=%d,bottom=%d,rotate=%d",
			iFB_W,height,start_x,start_y,start_x+width-1,start_y+height,iRot);
		cfa_conv_chk = eink_color_mapping_AIE(color_buffer,iEinkColorFormat,gray_buffer,
				iFB_W,iFB_H,c_buffer,cfa_aie_s4_bin,start_x,start_y,start_x+width-1,start_y+height,iRot);
		break;
#endif //] HWTCON_CFA_MODE_EINK_AIE_S4
#ifdef HWTCON_CFA_MODE_EINK_AIE_S7 //[
	case HWTCON_CFA_MODE_EINK_AIE_S7:
		CFA_LOG("width=%d,height=%d,left=%d,top=%d,right=%d,bottom=%d,rotate=%d",
			iFB_W,height,start_x,start_y,start_x+width-1,start_y+height,iRot);
		cfa_conv_chk = eink_color_mapping_AIE(color_buffer,iEinkColorFormat,gray_buffer,
				iFB_W,iFB_H,c_buffer,cfa_aie_s7_bin,start_x,start_y,start_x+width-1,start_y+height,iRot);
		break;
#endif //] HWTCON_CFA_MODE_EINK_AIE_S7
#ifdef HWTCON_CFA_MODE_EINK_AIE_S9 //[
	case HWTCON_CFA_MODE_EINK_AIE_S9:
		CFA_LOG("width=%d,height=%d,left=%d,top=%d,right=%d,bottom=%d,rotate=%d",
			iFB_W,height,start_x,start_y,start_x+width-1,start_y+height,iRot);
		cfa_conv_chk = eink_color_mapping_AIE(color_buffer,iEinkColorFormat,gray_buffer,
				iFB_W,iFB_H,c_buffer,cfa_aie_s9_bin,start_x,start_y,start_x+width-1,start_y+height,iRot);
		break;
#endif //] HWTCON_CFA_MODE_EINK_AIE_S9
...
COLOR_UPPER:
		...
		cfa_color_handle(cfa_color_input2, cfa_color_output2, 
			cfa_color_rec2.width,cfa_color_rec2.height,cfa_color_info_output2,
			1,1,cfa_color_rec2.x,cfa_color_rec2.y);
#endif 
		up(&color_lower);
...
static void cfa_calculate_color_region(struct rect input_rect, struct rect *output_rect1,
	struct rect *output_rect2, char **input2,  char **output2 , char **cinfo2)
{
...
*/
	...
	//	*output2 = hwtcon_fb_info()->color_buffer_va + hwtcon_fb_get_width() * buf_offset;
		...
		*cinfo2 =   hwtcon_fb_info()->cinfo_buffer_va;
	}
...
COLOR_WAIT:
			...
			cfa_fb_region.height < 100)
		{
			cfa_color_handle(hwtcon_fb_info()->fb_buffer_va, 
				...
				cfa_fb_region.width, cfa_fb_region.height,
				hwtcon_fb_info()->cinfo_buffer_va,
			   	1,0 ,cfa_fb_region.x,cfa_fb_region.y);
		} else {

			cfa_calculate_color_region(cfa_fb_region, &cfa_color_rec1, &cfa_color_rec2,
				&cfa_color_input2,  &cfa_color_output2 , &cfa_color_info_output2 );
			CFA_LOG("CFA input2[%p ] output2[%p ] cinfo2_output[%p ]",
				cfa_color_input2, cfa_color_output2, cfa_color_info_output2);
			...
			cfa_color_handle(hwtcon_fb_info()->fb_buffer_va, 
				hwtcon_fb_info()->color_buffer_va,
				cfa_color_rec1.width, cfa_color_rec1.height,
				hwtcon_fb_info()->cinfo_buffer_va,
				1,0, cfa_color_rec1.x,cfa_color_rec1.y);
