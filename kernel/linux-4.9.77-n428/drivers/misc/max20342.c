...
const int giMoistReportMaxCnt=1000;
...
static int max20342_moist_detect(unsigned int dwDetectFlags)
{
	if( dwDetectFlags & 
		(NTX_MOIST_DETFLAG_FORCE_MAN_DETECT|
		NTX_MOIST_DETFLAG_FORCE_AUTO_DETECT|
		NTX_MOIST_DETFLAG_FORCE_PERIOD_DETECT|
		NTX_MOIST_DETFLAG_FORCE_DIS_DETECT) 
		) 
	{
		unsigned char val = 0 ,old_val;
		const int wait_completion_timeout_ms=1350;

		max20342_read_reg(max20342_board_data.client, MAX20342_MOISTDET_CTRL, &val);
		old_val=val;
		val &= ~(MOISTDET_CTRL_MAN_EN|MOISTDET_CTRL_PER_EN|MOISTDET_CTRL_AUTO_EN) ;

		if(dwDetectFlags &NTX_MOIST_DETFLAG_FORCE_DIS_DETECT) {
		}
		else {
			if(dwDetectFlags &NTX_MOIST_DETFLAG_FORCE_MAN_DETECT) {
				val |= MOISTDET_CTRL_MAN_EN ;
			}
			else if(dwDetectFlags &NTX_MOIST_DETFLAG_FORCE_AUTO_DETECT) {
				val |= MOISTDET_CTRL_AUTO_EN ;
			}
			else {
				val |= (MOISTDET_CTRL_AUTO_EN|MOISTDET_CTRL_PER_EN) ;
			}

			if(dwDetectFlags &NTX_MOIST_DETFLAG_FORCE_PERIOD_DETECT) {
				val |= MOISTDET_CTRL_PER_EN ;
			}
		}

		MAXIAM_DBG("write moist_ctrl(0x%02x) 0x%02x => 0x%02x \n",
				MAX20342_MOISTDET_CTRL,old_val,val);

		max20342_write_reg(max20342_board_data.client, MAX20342_MOISTDET_CTRL, val);
		if(dwDetectFlags & NTX_MOIST_DETFLAG_WAIT_FORCE_DET) {
			reinit_completion(&max20342_board_data.irqwork_completion);
			if(0==wait_for_completion_timeout(&max20342_board_data.irqwork_completion,msecs_to_jiffies(wait_completion_timeout_ms))) {
				pr_err("%s() wait irqwork competion %d ms timeout \n",__func__,wait_completion_timeout_ms);
			}
		}
	}
	return max20342_board_data.iMoistDetectedLvl;
}
...
static void max20342_moist_det_work_func(struct work_struct *work)
{

	unsigned char bResInt2;
	const int iMoistIndetectedDebunceMax=NTX_MOIST_INDETECT_LVL;

	max20342_read_reg(max20342_board_data.client, MAX20342_RES_INT2, &bResInt2);

	if( bResInt2 & (0x01 | 0x08) ) {
		MAXIAM_DBG("moist detected again !? ResInt2=0x%02x\n",bResInt2);
		//cancel_delayed_work_sync(&max20342_board_data.moist_det_work);
		//schedule_delayed_work(&max20342_board_data.moist_det_work, msecs_to_jiffies(3000));
	}

	if(max20342_board_data.iMoistDetectedLvl>0) {
		max20342_board_data.iMoistDetectedLvl=0;
		MAXIAM_DBG("moist indetected ! ResInt2=0x%02x\n",bResInt2);
		max20342_moist_detect(NTX_MOIST_DETFLAG_FORCE_MAN_DETECT);
		//cancel_delayed_work_sync(&max20342_board_data.moist_det_work);
		schedule_delayed_work(&max20342_board_data.moist_det_work, msecs_to_jiffies(3000));
	}
	else if (max20342_board_data.iMoistDetectedLvl<=iMoistIndetectedDebunceMax) {
		// do nothing , and stop detect moist . 
	}
	else {
		max20342_board_data.iMoistDetectedLvl--;
		if(iMoistIndetectedDebunceMax==max20342_board_data.iMoistDetectedLvl) {
			MAXIAM_DBG("moist indetected level %d,ResInt2=%02x,switch to manual detection&periodic mode .\n",
				max20342_board_data.iMoistDetectedLvl,bResInt2);
			max20342_moist_detect(NTX_MOIST_DETFLAG_FORCE_MAN_DETECT|NTX_MOIST_DETFLAG_FORCE_PERIOD_DETECT);

			ntx_report_event(EV_MSC,MSC_RAW,MSC_RAW_MOIST_INDETECTED);
		}
		else {
			MAXIAM_DBG("moist indetected level %d,ResInt2=%02x.\n",
				max20342_board_data.iMoistDetectedLvl,bResInt2);
			max20342_moist_detect(NTX_MOIST_DETFLAG_FORCE_MAN_DETECT);
		}
		schedule_delayed_work(&max20342_board_data.moist_det_work, msecs_to_jiffies(2000));
	}
}
...
#if 0
	MAXIAM_DBG("MAX20342 RES_INT1=0x%02x,RES_INT2=0x%02x\n",
		buf[MAX20342_RES_INT1], buf[MAX20342_RES_INT2]);
#endif

	if ( (buf[MAX20342_RES_INT2]&(0x08|0x01)) || !max20342_board_data.is_hw_enabled ) 
	{
		// moist detected .
		unsigned char bADC_Result_Avg,bADCCtrl2;
		int iIpuResult;
		max20342_read_reg(max20342_board_data.client, MAX20342_MOIST_ADCRESULT_AVG, &bADC_Result_Avg);
		max20342_read_reg(max20342_board_data.client, MAX20342_ADC_CTRL2, &bADCCtrl2);
		iIpuResult = (bADCCtrl2>>5)&0x3;

		if(max20342_board_data.iMoistDetectedLvl<0) {
			max20342_board_data.iMoistDetectedLvl = 0;  
		}
		else if (max20342_board_data.iMoistDetectedLvl>giMoistReportMaxCnt) {
			// do not increse detecting level .
			max20342_board_data.iMoistDetectedLvl = giMoistReportMaxCnt;
		}
		else {
			max20342_board_data.iMoistDetectedLvl++;
		}

		MAXIAM_DBG("moist detected ! res_int2=0x%02x, lvl=%d , adc result=0x%02x, adc ctrl2=0x%02x(IpuResult=%duA)\n",
				buf[MAX20342_RES_INT2],
				max20342_board_data.iMoistDetectedLvl,
				bADC_Result_Avg,bADCCtrl2,giuAIpuResultA[iIpuResult]);

		if(max20342_board_data.iMoistDetectedLvl>0) {
			ntx_report_event(EV_MSC,MSC_RAW,MSC_RAW_MOIST_DETECTED);
		}

		
		//if (max20342_board_data.iMoistDetectedLvl<=giMoistReportMaxCnt) 
		{
			max20342_moist_detect(NTX_MOIST_DETFLAG_FORCE_MAN_DETECT);
...
exit:
	...
	if(!completion_done(&max20342_board_data.irqwork_completion)) {
		complete_all(&max20342_board_data.irqwork_completion);
	}
...
static int max20342_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	init_completion(&max20342_board_data.irqwork_completion);
	...
	INIT_DELAYED_WORK(&max20342_board_data.moist_det_work, max20342_moist_det_work_func);
	...
	max20342_read_reg(max20342_board_data.client, MAX20342_RES_INT2, &val);

	max20342_board_data.is_hw_ready = 1;
	ntx_charger_register_cc_detector(max20342_get_charger_type);
	ntx_charger_register_moist_detector(max20342_moist_detect);
	//max20342_moist_detect(NTX_MOIST_DETFLAG_FORCE_AUTO_DETECT|NTX_MOIST_DETFLAG_FORCE_PERIOD_DETECT);
	max20342_moist_detect(NTX_MOIST_DETFLAG_FORCE_MAN_DETECT|NTX_MOIST_DETFLAG_FORCE_PERIOD_DETECT);
...
#if 0
	if (delayed_work_pending(&max20342_board_data.moist_det_work)) {
		printk(KERN_ERR"%s skipped ! moist_det_work is pending !\n",__func__);
		return -1;
	}
#endif
