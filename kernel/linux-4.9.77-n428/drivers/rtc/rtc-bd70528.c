...
int bd7xx28_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct bd7xx28_rtc *bd_rtc;
	struct rtc_time t;


	bd_rtc = platform_get_drvdata(pdev);
	if(!bd_rtc) {
		pr_err("%s() bd_rtc getting failed !\n",__func__);
		return 0;
	}

	pr_debug("%s time : \n", __func__);
	bd7xx28_get_time(&pdev->dev,&t);
	
	return 0;
}


int bd7xx28_resume(struct platform_device *pdev)
{
	struct bd7xx28_rtc *bd_rtc;
	struct rtc_time t;
...
static struct platform_driver bd70528_rtc = {
	...
	.probe = bd7xx28_probe,
	.suspend = bd7xx28_suspend,
	.resume = bd7xx28_resume, 
};
