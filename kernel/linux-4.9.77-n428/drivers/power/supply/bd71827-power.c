...
/* 0xfb record power off status (Ex:dcin...etc)*/
#define BD7182x_POWER_OFF_STATUS	0xFB
#define BD7182x_POWER_OFF_MASK_DCIN	0x01
...
#define SOC_90_MAX_CURR_PR284983N	350000 // 350mA
...
struct soc_curr_table calc_soc_table_PR284983N[6] = {
	{95, 212},
	{96, 185},
	{97, 157},
	{98, 130},
...
struct bd71827_power {
	...
	int iForcePowerCtrlMoistDetected;
};
...
static int bd71827_calc_soc(struct bd71827_power *pwr, struct bd7182x_soc_data *wd);
...
{
	static int flag=0, off_status = 0;
	...
	regmap_read(pwr->mfd->regmap, BD7182x_POWER_OFF_STATUS, &off_status);
	off_status &= 0xfe;
	if (pwr->charger_type != BD_71879_CHARGER_TYPE_NONE)  // record dcin status
		off_status |= BD7182x_POWER_OFF_MASK_DCIN;
	else
		off_status &= BD7182x_POWER_OFF_MASK_DCIN;
	regmap_write(pwr->mfd->regmap, BD7182x_POWER_OFF_STATUS, off_status);
}
...
#endif
static int bd71827_charge_enable(struct bd71827_power *pwr,int iEn)
{
	int iRet = 0;
	...
	if (iEn == 1) {
		iRet = regmap_update_bits(pwr->mfd->regmap, pwr->regs->chg_en,
...
static int EnableDCINCharger(struct bd71827_power *pwr)
{
	...
	ret = regmap_read(pwr->mfd->regmap, BD71828_REG_BAT_STAT, &val);
	if(ret)
		return ret;
	if(val & (BAT_DET_DONE|BAT_DET)) {
		ret = regmap_read(pwr->mfd->regmap, BD71828_REG_DCIN_SET, &val);
		if(ret)
			return ret;
		val |= 0x80;
		ret = regmap_write(pwr->mfd->regmap, BD71828_REG_DCIN_SET, val);
				 ...
				 struct bd7182x_soc_data *wd)
{
	int ret = 0;
	u32 cc;
	int sorg, soc_cal, status, off_status = 0;
	...
	if (wd->state_machine == STAT_POWER_ON) {
		...
		mdelay(500);
		/*bd71827_get_init_bat_stat(pwr, &temp);
		soc_cal = bd71827_voltage_to_capacity(temp)/10;
		wd->vcell = temp;
		ocv = wd->vcell;*/

		/* Get soc at relaxed state from ocv/soc table */
		soc_cal = bd71827_voltage_to_capacity(wd->vcell)/10;
		wd->soc_norm = sorg;
		bd71827_charge_status(pwr, wd, 0);
		bd71827_calc_soc(pwr, wd);
		regmap_read(pwr->mfd->regmap, pwr->regs->dcin_stat, &status);
		regmap_read(pwr->mfd->regmap, BD7182x_POWER_OFF_STATUS, &off_status);
		dev_err(pwr->mfd->dev,  "%s() sorg = %d, soc_cal = %d, voltage %d, soc %d, charge %d, dc %d, off_dc %d\n", __func__, sorg, soc_cal, wd->vcell, wd->soc, wd->rpt_status, status&BD7182x_MASK_DCIN_STAT, off_status&BD7182x_POWER_OFF_MASK_DCIN);
		if((((sorg+5) < soc_cal) && (sorg > 0)) || (off_status&BD7182x_POWER_OFF_MASK_DCIN)) {
		...
		// clr record power off dcin status
		off_status &= 0xfe;
		regmap_write(pwr->mfd->regmap, BD7182x_POWER_OFF_STATUS, off_status);
		/*else {
		...
		if (pwr->charger_type == BD_71879_CHARGER_TYPE_DCP && cc_calc_curve_en) {
			if (cc_calc_skip) {
				...
				if( (21==gptHWCFG->m_val.bBattery) || (26==gptHWCFG->m_val.bBattery)) {
					// DCP charge current limit = 1.5A
					if (wd->curr >= 1510000)
						cc_calc_skip = 1;
					else
						cc_calc_skip = 0;
				} else {
					// DCP charge current limit = 1.3A
					if (wd->curr >= 1300000)
						cc_calc_skip = 1;
					else
						cc_calc_skip = 0;
				 ...
				 struct bd7182x_soc_data *wd)
{
	...
	wd->temp = temp;
	dev_dbg(pwr->mfd->dev,  "Temperature = %d\n", wd->temp);
	bd71827_adjust_coulomb_count(pwr, wd, 0);
	...
	if (use_load_bat_params == 0) {
		switch(gptHWCFG->m_val.bBattery) {
			...
			case 14:	//PR-284983N-1500mA
				...
				curr_90_max = SOC_90_MAX_CURR_PR284983N;
				memcpy(&calc_soc_table, &calc_soc_table_PR284983N, sizeof(calc_soc_table_PR284983N));
				break;
			case 21: // 1200mAx2 .
...
int bd71828_charge_rsense_update(int mode)
{
	...
	wd = &pwr->d_w;
	if (mode != BD_71879_CHARGER_TYPE_NONE) {
...
static void bd_work_callback(struct work_struct *work)
{
	...
	if(ntx_charger_moist_detector_exist()) {
		if(ntx_charger_moist_detect(0)>0) {
			if(pwr->iForcePowerCtrlMoistDetected) {
				dev_info(pwr->mfd->dev, "%s : force enable charge ignore moist detected !\n",__func__);
				bd71827_charge_enable(pwr,1);
			}
			else {
				// disable power control . 
				dev_info(pwr->mfd->dev, "%s : force disable charge since moist detected !\n",__func__);
				bd71827_charge_enable(pwr,0);
			}
		}
		else {
	...
	if (wd->state_machine == STAT_POWER_ON)
		ret = bd71827_adjust_coulomb_count(pwr, wd, 1);
	else
...
static ssize_t bd71827_sysfs_show_moist(struct device *dev,
					    struct device_attribute *attr,
					    char *buf)
{
	ssize_t ret;
	...
	iMoistLvl = ntx_charger_moist_detect(0);
	ret = sprintf(buf,"%d\n",(iMoistLvl<0)?0:iMoistLvl);
	return ret;
}

static ssize_t bd71827_sysfs_store_moist(struct device *dev,
					   struct device_attribute *attr,
					   const char *buf,
					   size_t count)
{
	int ret;
	const int iWaitDefault=0,iPeriodicDefault=0;
	int iEn,iWait=iWaitDefault,iPeriodic=iPeriodicDefault;
	int iMoist_chk;
	unsigned int dwFlags;

	pr_info("usage : <en_mode> [periodic] [wait]\n");
	pr_info("  <en_mode> : 1=manual detect, 2:auto detect , 0:disable detect\n");
	pr_info("  [periodic] : 1=periodic enable,default is %d\n",iPeriodicDefault);
	pr_info("  [wait] : 1=wait a while for settings effective,default is %d\n",iWaitDefault);
	pr_info("\n");

	ret = sscanf(buf, "%d %d %d", &iEn,&iPeriodic,&iWait);
	if(1==iEn) {
		dwFlags = NTX_MOIST_DETFLAG_FORCE_MAN_DETECT;
		if(1==iPeriodic) {
			dwFlags |= NTX_MOIST_DETFLAG_FORCE_PERIOD_DETECT;
		}
		if(1==iWait) {
			dwFlags |= NTX_MOIST_DETFLAG_WAIT_FORCE_DET;
		}
		iMoist_chk = ntx_charger_moist_detect(dwFlags);
		pr_info("%s :manual iMoist_chk=%d,iEn=%d,iPeriodic=%d,iWait=%d\n",
				__func__,iMoist_chk,iEn,iPeriodic,iWait);
	}
	else if(2==iEn) {
		dwFlags = NTX_MOIST_DETFLAG_FORCE_AUTO_DETECT;
		if(1==iPeriodic) {
			dwFlags |= NTX_MOIST_DETFLAG_FORCE_PERIOD_DETECT;
		}
		if(1==iWait) {
		...
		iMoist_chk = ntx_charger_moist_detect(dwFlags);
		pr_info("%s :auto iMoist_chk=%d,iEn=%d,iPeriodic=%d,iWait=%d\n",
				__func__,iMoist_chk,iEn,iPeriodic,iWait);
	}
	else if(0==iEn) {
		dwFlags = NTX_MOIST_DETFLAG_FORCE_DIS_DETECT;
		iMoist_chk = ntx_charger_moist_detect(dwFlags);
		pr_info("%s :disable iMoist_chk=%d,iEn=%d,iPeriodic=%d,iWait=%d\n",
				__func__,iMoist_chk,iEn,iPeriodic,iWait);
	}
	return count;
}

static DEVICE_ATTR(moist, 0644,
		bd71827_sysfs_show_moist, bd71827_sysfs_store_moist);

static ssize_t bd71827_sysfs_show_moist_force_powerctrl(struct device *dev,
					    struct device_attribute *attr,
					    char *buf)
{
	struct power_supply *psy = dev_get_drvdata(dev);
	...
	ret = sprintf(buf,"%d\n",pwr->iForcePowerCtrlMoistDetected);
	return ret;
}

static ssize_t bd71827_sysfs_store_moist_force_powerctrl(struct device *dev,
					   struct device_attribute *attr,
					   const char *buf,
					   size_t count)
{
	...
	ret = sscanf(buf, "%d", &iTemp);
	
	pwr->iForcePowerCtrlMoistDetected = iTemp?1:0;

	return count;
}

static DEVICE_ATTR(moist_force_powerctrl, 0644,
		bd71827_sysfs_show_moist_force_powerctrl, bd71827_sysfs_store_moist_force_powerctrl);
...
static ssize_t batt_params_read(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	len = sprintf(pcBuf,"charging curr 90%% = %d uA\n",curr_90_max);pcBuf += len;total_len += len;
...
static struct attribute *bd71827_sysfs_attributes[] = {
	...
	&dev_attr_moist.attr,
	&dev_attr_moist_force_powerctrl.attr,
...
void battery_stop_schedule(void)
{
	...
	pwr = platform_get_drvdata(gPwr_dev);
	pwr->schedule_work = 0;
	bd71828_charge_rsense_update(BD_71879_CHARGER_TYPE_NONE);
	cancel_delayed_work_sync(&pwr->bd_work);
	cancel_delayed_work_sync(&pwr->usb_work);
	//cancel_delayed_work(&pwr->bd_work);
}
