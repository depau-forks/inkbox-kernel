	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	} else if ((!strncmp(buf, "susdbg", 6))
		&& (sscanf(buf+6, "%d", &val) == 1)) {
		pctl->sus_gpiostatus = val;
	}
...
static void gpio_show_suspend_pin(void)
{
	if (pctl->sus_gpiostatus) {
		unsigned int i = 0;
		struct gpio_chip *chip;
		...
		chip = pctl->chip;
		pr_notice("gpio base: 0x%x, pins: %03d\n",
			chip->base, pctl->chip->ngpio);

		if (pctl->dbg_start >= pctl->chip->ngpio) {
			pr_notice("wrong gpio-range: start should less than %d!\n",
			pctl->chip->ngpio);
			return;
		}

		pr_notice("PIN: (MODE)(DIR)(DOUT)(DIN)(DRIVE)(SMT)(IES)(PULL_EN)(PULL_SEL)\n");

		for (i = pctl->dbg_start; i < pctl->chip->ngpio; i++) {
			pr_notice("%03d: %1d%1d%1d%1d%2d%1d%1d%1d%1d",
					i,
					mtk_pinmux_get(chip, i),
					!mtk_gpio_get_direction(chip, i),
					mtk_gpio_get_out(chip, i),
