...
static ssize_t kx122_sysfs_get_tilt_timer(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct kx122_data *sdata = dev_get_drvdata(dev);
	int res;
	...
	mutex_lock(&sdata->mutex);
	res = kx122_reg_read_byte(sdata, KX122_TILT_TIMER);
	mutex_unlock(&sdata->mutex);

	return sprintf(buf, "%x\n", res);
}

static ssize_t kx122_sysfs_set_tilt_timer(struct device *dev,
				struct device_attribute *attr,const char *buf, size_t count)
{
	struct kx122_data *sdata = dev_get_drvdata(dev);
	int err = 0 ;
	...
	err = kx122_reg_write_byte(sdata, KX122_TILT_TIMER, value);
...
static struct device_attribute dev_attr_tilt_timer = __ATTR(tilt_timer,
		S_IRUGO| S_IWUSR,
		kx122_sysfs_get_tilt_timer,
		kx122_sysfs_set_tilt_timer);
...
static int kx122_set_reg(struct kx122_data *sdata)
{
	...
	err = kx122_reg_write_byte(sdata, KX122_TILT_TIMER, sdata->Tilt_Timer);
	if(err)
		printk(KERN_ERR"[%s_%d] Write TILT_TIMER Failed\n",__FUNCTION__,__LINE__);
...
{	
	...
	val = kx122_reg_read_byte(sdata, KX122_INC1);
	printk("[kx122-INC1]:%x \n",val);
	val = kx122_reg_read_byte(sdata, KX122_INC2);
	printk("[kx122-INC2]:%x \n",val);
	val = kx122_reg_read_byte(sdata, KX122_INC3);
	printk("[kx122-INC3]:%x \n",val);
