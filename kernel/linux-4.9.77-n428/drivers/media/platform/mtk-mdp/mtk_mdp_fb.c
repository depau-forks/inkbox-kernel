...
void mtk_mdp_set_rotation(struct mtk_mdp_ctx *ctx,
				int rotate, int hflip, int vflip, unsigned char cfa)
{
	...
	misc->vflip = vflip;
	if (1==cfa)
		misc->alpha = 1 << 31;//regal
	else if (2==cfa)
		misc->alpha = 1 << 30;//color mapping
}
...
void mtk_mdp_set_pq_info(struct mtk_mdp_ctx *ctx, struct mdp_pq_info *pq, enum mtk_mdp_gamma_lut_id lut_id)
{
	...
	memcpy(vsi_pq->gamma_table, mdp_gamma_lut[lut_id],
		...
		u8 hflip, u8 vflip, u8 cfa)
{
	...
	MDP_LOG_INFO(
		"easy_mtk_mdp_func_user: %ux%u->%ux%u, %s->%s, %u->%u, 0x%x 0x%x [%u %u %u %u] [%u 0x%x %u %u] F:%d,%d cfa mode:%d\n",
	...
	src_frame.crop.width = src_w;
	src_frame.crop.height = src_h;
	...
	switch (rotate) {
		case 0:
		case 180:
			dst_frame.crop.width = src_w;
			dst_frame.crop.height = src_h;
			dst_frame.width = src_w;
			dst_frame.height = src_h;
			break;
		case 90:
		case 270:
			dst_frame.crop.width = src_h;
			dst_frame.crop.height = src_w;
			dst_frame.width = src_h;
			dst_frame.height = src_w;
			break;
		default:
	...
	dst_frame.payload[0] = dst_pitch * dst_h + 4095;
	...
	// gamma map setting for userspace mdpd
	...
	mtk_mdp_set_rotation(ctx, rotate, hflip, vflip, cfa);
	mtk_mdp_set_pq_info(ctx, &pq, lut_id);
...
int easy_mtk_mdp_func_v2(
		...
		u32 crop_w, u32 crop_h,
		u8 dth_en, u32 dth_algo, u8 invert,
		u32 rotate, u32 gamma_flag, u8 hflip, u8 vflip, u8 cfa)
{
	bool user_kernel = false;
    char *src_fmt_string = fmt_2_string(src_fmt);
	char *dst_fmt_string = fmt_2_string(dst_fmt);

	MDP_LOG_INFO(
	"easy_mtk_mdp_func_v2: %ux%u->%ux%u, %s->%s, %u->%u, 0x%x 0x%x [%u %u %u %u] [%u 0x%x %u %u] gamma_flag:%u, F:%d,%d cfa mode:%d\n",
			src_w, src_h, dst_w, dst_h,
			fmt_2_string(src_fmt), fmt_2_string(dst_fmt),
			src_pitch, dst_pitch,
			src_mva, dst_mva,
			crop_x, crop_y, crop_w, crop_h,
			dth_en, dth_algo, invert, rotate,
			gamma_flag, hflip, vflip, cfa);

	if ((strcmp(src_fmt_string, "Unknown fmt") == 0) ||
	    (strcmp(dst_fmt_string, "Unknown fmt") == 0)) {
			MDP_LOG_INFO("easy_mtk_mdp_func_v2: Format is unknown, mdp is not support, return\n");
			return -EINVAL;
	}
	...
	if (get_mdp_service_init() == 0 || user_kernel) {
