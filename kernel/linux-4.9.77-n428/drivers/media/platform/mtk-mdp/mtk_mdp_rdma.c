		 ...
		 u32 crop_type, u32 crop_x, u32 crop_y, u32 crop_w, u32 crop_h)
{
	...
	if (fmt == V4L2_PIX_FMT_ARGB32 || fmt == V4L2_PIX_FMT_RGB565 || fmt == V4L2_PIX_FMT_RGB24 ||
	    fmt == V4L2_PIX_FMT_ABGR32)
		MDP_REG_MASK(MDP_RDMA_TRANSFORM_0, 0x1010000,  0xf010000);
	else
	...
	if (crop_type == 1) {
		...
		else if (fmt == V4L2_PIX_FMT_ABGR32)
			MDP_REG_SET(MDP_RDMA_SRC_BASE_0, addr + crop_y * pitch + crop_x * 4);
		else if (fmt == V4L2_PIX_FMT_RGB24)
			MDP_REG_SET(MDP_RDMA_SRC_BASE_0, addr + crop_y * pitch + crop_x * 3);
		else
	...
	if (fmt == V4L2_PIX_FMT_Y8) {
		...
		MDP_REG_MASK(MDP_RDMA_SRC_CON, 0x18127240 | 0x2, 0xffffffff);
		MDP_REG_MASK(MDP_RDMA_Y4_MODE_CFG, 0x0, 0xf);
	} else if (fmt == V4L2_PIX_FMT_ABGR32) {
		MDP_REG_MASK(MDP_RDMA_SRC_CON, 0x18127240 | 0x3, 0xffffffff);
		MDP_REG_MASK(MDP_RDMA_Y4_MODE_CFG, 0x0, 0xf);
	} else if (fmt == V4L2_PIX_FMT_RGB24) {
		MDP_REG_MASK(MDP_RDMA_SRC_CON, 0x18127240 | 0x1, 0xffffffff);
		MDP_REG_MASK(MDP_RDMA_Y4_MODE_CFG, 0x0, 0xf);
