...
void connect_path1(void)
{
	MDP_LOG_INFO("mdp_path1: RDMA->GAMMA->DITHER->WROT\n");
	MDP_REG_MASK(MDP_CONFIG_RDMA0_MOUT_EN, 0x10, 0x1F); /* 0xF08 */
	MDP_REG_MASK(MDP_CONFIG_WROT0_SEL_IN, 0x01, 0x7);  /* 0xF38 */
}

void connect_path0(void)
{
	MDP_LOG_INFO("mdp_path0: RDMA->GAMMA->DITHER->WROT\n");
