...
struct bd71828_leds {
	...
	unsigned int green_en;
	struct bd71828_led amber;
	unsigned int amber_en;
};
...
int bd71828_led_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if(1==gBd71828Leds->amber_en) {
		flush_work(&gBd71828Leds->amber.led_work);
	}

	if(1==gBd71828Leds->green_en) {
...
static int bd71828_led_probe(struct platform_device *pdev)
{
	...
	int ret = 0;
	int chk = 0;
	unsigned int led_green_en,led_amber_en;
	

	pr_info("bd71828 LED driver probed\n");

	chk = of_property_read_u32(pdev->dev.parent->of_node,"led_green_en", &led_green_en);
	if (chk) {
		pr_info("led_green_en dt property error(%d),enabled default\n",chk);
		led_green_en = 1;
	}
	chk = of_property_read_u32(pdev->dev.parent->of_node,"led_amber_en", &led_amber_en);
	if (chk) {
		pr_info("led_amber_en dt property error(%d),enabled default\n",chk);
		led_amber_en = 1;
	}


	if(0==led_amber_en && 0==led_green_en) {
		pr_info("bd71828 LED driver skipped since leds is disabled \n");
		return -1;
	}
