...
static int fl_brightness_not_zero = 0;	// If percentage is larger than 0 , set 1
...
static int lm3630a_bank_a_update_status(struct backlight_device *bl)
{
...
#else
				if(gpio_is_valid(pchip->hwen_gpio)) {
					printk("%s_%d(): Hiber FL EN [OFF] \n",__FUNCTION__,__LINE__);
					gpio_direction_output(pchip->hwen_gpio, 0);

					if(gpio_is_valid(pchip->pwr_on_gpio)){
						printk("%s_%d(): Hiber FL PWR[OFF] \n",__FUNCTION__,__LINE__);
						gpio_direction_output(pchip->pwr_on_gpio, 0);
					}
					fl_en_status = 0;
				}
				else{
					printk("%s(): gpio_is_invalid (hwen_gpio)\n",__FUNCTION__);
				}
#endif
			}
...
#else
			printk("[%s_%d] fl_brightness_not_zero:%d , fl_en_status:%d\n",__FUNCTION__,__LINE__,fl_brightness_not_zero,fl_en_status);
			if( 0==fl_brightness_not_zero ){
				if(gpio_is_valid(pchip->hwen_gpio)) {
					printk("%s_%d():FL EN [OFF] \n",__FUNCTION__,__LINE__);
					gpio_direction_output(pchip->hwen_gpio, 0);
					if(gpio_is_valid(pchip->pwr_on_gpio)){
						printk("%s_%d():FL PWR [OFF] \n",__FUNCTION__,__LINE__);
						gpio_direction_output(pchip->pwr_on_gpio, 0);
					} 
					fl_en_status = 0;
				}
	...
	if (giSuspendingState && gSleep_Mode_Suspend) {
		...
		if (giFL_PWR_state==1) {
			if(gpio_is_valid(pchip->pwr_on_gpio)) {
				printk("%s_%d():FL PWR [ON] \n",__FUNCTION__,__LINE__);
				gpio_direction_output(pchip->pwr_on_gpio, 1);
			}
			if(gpio_is_valid(pchip->hwen_gpio)) {
				printk("%s_%d():FL EN [ON] \n",__FUNCTION__,__LINE__);
		...
		unsigned char led_B_current, unsigned char led_B_brightness)
{
	...
	if( 0==led_A_brightness && 0==led_B_brightness)
		fl_brightness_not_zero = 0;
	else
		fl_brightness_not_zero = 1;
